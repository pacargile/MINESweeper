...
# Read in demo info
inspec = Table.read('./spectra/HARPS.Archive_18Sco_R20K.fits',format='fits')
...
inputdict['spec']['obs_eflux'] = np.array(
    [np.sqrt(x**2.0 + (0.01*y)**2.0) for x,y in zip(inspec['EFLUX'],inspec['FLUX'])])
...
inputdict['priordict']['Inst_R'] = ({'fixed':20000.0})
...
inputdict['output'] = 'MSoutput/HARPS.Archive_18Sco_R20K.dat'
